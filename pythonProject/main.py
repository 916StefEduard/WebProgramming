...
class DMap():
    def __init__(self):
        ...
        for i in range(self.__n):
            for j in range(self.__m):
                self.surface[i][j] = -1
        self.visited = [[False for _ in range(20)] for _ in range(20)]
...
class Drone():
    ...
    def moveDSF(self, detectedMap):
        ...
        if 0 <= self.x <= 19 and 0 <= self.y-1 <= 19 and detectedMap.surface[self.x][self.y-1] != 1:
           if not self.visited[self.x][self.y-1] :
                self.stack.append([self.x,self.y-1])
                self.visited[self.x][self.y-1] = True
                self.x = self.x
                self.y = self.y-1
                return
        if 0 <= self.x+1 <= 19 and 0 <= self.y <= 19 and detectedMap.surface[self.x+1][self.y] != 1:
           if not self.visited[self.x+1][self.y]:
                self.stack.append([self.x+1,self.y])
                self.visited[self.x+1][self.y] = True
                self.x=self.x+1
                self.y=self.y
                return
        if 0 <= self.x <= 19 and 0 <= self.y+1 <= 19 and detectedMap.surface[self.x][self.y+1] != 1:
...
def main():
    ...
    e = Environment()
    e.loadEnvironment("Images/test2.map")
    # print(str(e))
